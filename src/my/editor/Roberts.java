...
public class Roberts {
    ...
    public Roberts(BufferedImage img){
        this.l = img.getWidth();
    ...
    public BufferedImage robertsArit(){
        ...
        for(int i=1; i<l-1; i++){
            for(int j=1; j<a-1; j++){
                double tempRedx = finalArray[i][j][0]*0 + finalArray[i+1][j][0]*1 + finalArray[i+1][j-1][0]*0 + finalArray[i][j-1][0]*(-1);
                double tempRedy = finalArray[i][j][0]*1 + finalArray[i+1][j][0]*0 + finalArray[i+1][j-1][0]*(-1) + finalArray[i][j-1][0]*0;
                ...
                double tempGreenx = finalArray[i][j][1]*0 + finalArray[i+1][j][1]*1 + finalArray[i+1][j-1][1]*0 + finalArray[i][j-1][1]*(-1);
                double tempGreeny = finalArray[i][j][1]*1 + finalArray[i+1][j][1]*0 + finalArray[i+1][j-1][1]*(-1) + finalArray[i][j-1][1]*0;
                ...
                double tempBluex = finalArray[i][j][2]*0 + finalArray[i+1][j][2]*1 + finalArray[i+1][j-1][2]*0 + finalArray[i][j-1][2]*(-1);
                double tempBluey = finalArray[i][j][2]*1 + finalArray[i+1][j][2]*0 + finalArray[i+1][j-1][2]*(-1) + finalArray[i][j-1][2]*0;
